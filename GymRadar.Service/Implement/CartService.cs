    ...
    public class CartService : BaseService<CartService>, ICartService
    {
        ...
        public async Task<BaseResponse<CreatePaymentResult>> CreatePaymentUrlRegisterCreator(CreateQRRequest request)
        {
            ...
            var transaction = new Model.Entity.Transaction
            {
                ...
                PtId = request.PTId,
                Type = TypeTransactionEnum.Course.GetDescriptionFromEnum(),
        ...
        public async Task<BaseResponse<CreatePaymentResult>> CreatePaymentNotPT(CreateQRNotPTRequest request)
        {
            Guid? id = UserUtil.GetAccountId(_httpContextAccessor.HttpContext);
            var account = await _unitOfWork.GetRepository<Account>().SingleOrDefaultAsync(
                predicate: u => u.Id.Equals(id) && u.Active == true);
        ...
        public async Task<BaseResponse<CreatePaymentResult>> CreatePaymentUrlPremium(CreateQRPremiumRequest request)
        {
            Guid? id = UserUtil.GetAccountId(_httpContextAccessor.HttpContext);
            var account = await _unitOfWork.GetRepository<Account>().SingleOrDefaultAsync(
                predicate: u => u.Id.Equals(id) && u.Active == true);
            ...
            var premium = await _unitOfWork.GetRepository<Premium>().SingleOrDefaultAsync(
                predicate: gc => gc.Id.Equals(request.Id) && gc.Active == true);
            ...
            if (premium == null)
            {
                ...
                {
                    status = StatusCodes.Status404NotFound.ToString(),
                    message = "Không tồn tại gói premium này",
                    data = null
                };
            }
            ...
            var transaction = new Model.Entity.Transaction
            {
                ...
                Description = description,
                UserId = user.Id,
                PremiumId = request.Id,
                Type = TypeTransactionEnum.Premium.GetDescriptionFromEnum(),
                CreateAt = TimeUtil.GetCurrentSEATime(),
            };
            await _unitOfWork.GetRepository<Model.Entity.Transaction>().InsertAsync(transaction);
            await _unitOfWork.CommitAsync();
